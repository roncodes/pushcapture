...
class Auth extends MY_Controller {
	...
	function index()
	{
		...
		{
			/* User is admin, redirect them to admin */
			echo 'home/dashboard';
		} 
		else 
		{
			/* User is logged in, redirect them to home screen */
			echo 'home/dashboard';
		}
	}
	...
	function sign_in()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), true))
			{
				flashmsg('You have logged in, welcome back!', 'success');
				redirect(base_url());
			}
			else
			{
		...
		{
			flashmsg(validation_errors(), 'error');
		}
	}
	...
	function logout()
	{
		...
		flashmsg('You have logged out, good bye!', 'info');
		redirect('auth/sign_in');
	}
